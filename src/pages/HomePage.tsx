...
export function HomePage({ onSearch }: HomePageProps) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 pt-[15vh]">
      {/* Title */}
      <motion.div
      ...
      >
        ...
        <p className="mt-2 text-[var(--text-secondary)] text-[14px]">
      ...
      >
        {SUGGESTIONS.map((s, i) => (
          <motion.button
            ...
            className="btn btn-outline text-[12px] gap-1"
          >
      ...
      {searchHistory.length > 0 && (
        ...
        >
          ...
          <div className="border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-card)] divide-y divide-[var(--border-subtle)] overflow-hidden">
            {searchHistory.slice(0, 4).map((h, i) => (
              <motion.button
                ...
                className="w-full flex items-center justify-between px-3.5 py-2.5 hover:bg-[var(--bg-secondary)] transition-colors group text-left"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <Search size={12} className="text-[var(--text-tertiary)] shrink-0" />
                  <span className="text-[13px] truncate">{h.query}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[11px] text-[var(--text-tertiary)]">{h.result_count}건</span>
                  <ArrowRight
                    size={11}
