...
| AI 요약 | 검색 결과 상단에 AI가 요약을 생성 |
| 자동완성 | 입력 중 히스토리 + 인기 토픽 기반 추천 |
| 키보드 탐색 | j/k로 결과 이동, Enter로 미리보기 |
| 저장소 미리보기 | README, 통계, 토픽을 모달로 확인 |
| 건강도 분석 | 활동성/커뮤니티/문서화 기반 점수 + 등급 |
| 저장소 비교 | 2~3개 저장소를 나란히 비교 |
| 코드 Q&A | 저장소 코드에 대해 AI에게 질문 |
| 컬렉션 | 즐겨찾기 저장소를 폴더+메모로 관리 |
| 트렌딩 | 오늘/이번주 인기 저장소 + 스타 증가량 |
| 통계 대시보드 | 내 검색 패턴, 키워드, 활동 그래프 |
| 다크/라이트 모드 | 테마 전환 + 6가지 커스텀 색상 |
| 온보딩 | 첫 실행 시 3단계 설정 가이드 |
| 시스템 트레이 | 최소화 시 트레이 상주, 더블클릭 복원 |
| 다국어 | 한국어, 영어, 일본어 UI |
| 코드 스니펫 저장 | Q&A에서 나온 코드를 서랍에 저장 |
| 오프라인 모드 | 인터넷 없이 캐시된 저장소 검색 |
| 로컬 AI | Ollama로 무료 로컬 AI 사용 |
| 설정 | GitHub 토큰, AI 키, 테마, 언어 한곳에서 관리 |
