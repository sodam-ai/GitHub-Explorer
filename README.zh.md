...
| 功能 | 说明 |
|------|------|
| AI搜索 | 用自然语言输入，从仓库、代码、Issue中智能搜索 |
| AI摘要 | 搜索结果顶部由AI生成摘要 |
| 自动补全 | 输入时显示历史和热门话题建议 |
| 键盘导航 | j/k移动结果，Enter预览 |
| 仓库预览 | 在弹窗中查看README、统计和标签 |
| 健康度分析 | 基于活跃度/社区/文档的评分和等级 |
| 仓库比较 | 并排比较2-3个仓库 |
| 代码问答 | 向AI提问仓库代码相关问题 |
| 收藏夹 | 用文件夹+备注管理喜欢的仓库 |
| 趋势 | 今日/本周热门仓库和Star增长 |
| 统计面板 | 搜索模式、关键词、活动图表 |
| 暗色/亮色模式 | 主题切换 + 6种自定义颜色 |
| 引导教程 | 首次启动时3步设置向导 |
| 系统托盘 | 最小化到托盘，双击恢复 |
| 多语言 | 韩语、英语、日语UI |
| 代码片段保存 | 将问答中的代码保存到抽屉 |
| 离线模式 | 无网络时搜索缓存数据 |
| 本地AI | 通过Ollama使用免费本地AI |
| 设置 | 在一处管理GitHub令牌、AI密钥、主题和语言 |
...
| Phase 2 | 代码问答、书签、收藏夹、健康度分析、比较、智能文件夹、代码查看器、筛选器 | **已完成**（9/9） |
| Phase 3 | 本地AI（Ollama）、离线模式、趋势仪表盘、导出、自动更新、缓存策略 | **已完成**（8/8） |
