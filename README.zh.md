...
| 阶段 | 功能 | 状态 |
|------|------|------|
| Phase 1 (MVP) | AI 搜索、AI 摘要、标签页、历史记录、快捷键、深色模式、设置、SQLite、GitHub 认证 | 核心功能可用* |
| Phase 2 | 代码问答、书签、收藏集、健康度分析、对比、智能文件夹、代码查看器、筛选器 | 大部分可用，部分未完成* |
| Phase 3 | 本地 AI (Ollama)、离线模式、趋势面板、导出、自动更新、缓存策略 | 核心功能可用，部分未实现* |

\* GitHub 认证使用 Personal Access Token，而非 OAuth App。代码问答基于 README 和文件列表，而非向量搜索(RAG)。智能推荐引擎和自动更新尚未实现。（截至 2026-07-20 代码审计）

---

## 安全性

- API 密钥仅存储在 **您电脑的操作系统密钥链**（如 Windows 凭据管理器）中，不会存入数据库或浏览器存储
